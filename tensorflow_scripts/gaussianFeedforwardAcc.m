fileID_acc = fopen('results_ori/ori_acc.txt','r');
fileID_norm = fopen('results_ori/ori_norm.txt','r');
...
xpos = linspace(0,20,acc_data_points);
figure(f_acc);

%plot(xpos, acc_trials,'-go', xpos, lam5_acc_trials, '-ro', xpos, lam4_acc_trials, '-bo');
plot(xpos, acc_trials,'-go', xpos, lam5_acc_trials, '-ro');
...
plot(xpos, norm_trials,'-go', xpos,lam5_norm_trials, '-ro');
